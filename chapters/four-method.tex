...
\end{itemize}
برای نمونه در سیستم تخلیه‌ی وظیفه‌ای با ویژگی‌های مشخص شده در جدول \ref{table:fixedranged2} احتمال انتقال به حالات بعدی مطابق با جدول \ref{table:transitions} می‌باشد. برای سادگی، در اینجا از توضیح بیشتر در مورد نحوه محاسبه مقادیر درایه‌های ماتریس انتقال صرف نظر می‌کنیم. برای آگاهی از نحوه محاسبه این مقادیر در قالب کد به \hyperref[appendix:3]{پیوست ۳} رجوع شود.
\begin{table}[H]
	\centering
	\begin{latin}
...
به منظور محاسبه معیارهای توان مصرفی میانگین و تاخیر سرویس میانگین لازم است که بتوانیم درباره وضعیت سیستم تخلیه‌ی وظیفه در طولانی‌مدت استنتاج کنیم. در همین راستا مفهوم \textbf{توزیع پایدار} را برای زنجیره مارکوف تعریف می‌کنیم.
\begin{defi}
...
\end{thm}
حال با استفاده از قضیه \ref{thm:converge} ثابت می‌کنیم که زنجیره مارکوف سیستم تخلیه‌ی وظیفه دارای توزیع پایدار منحصر به فرد است. برای سادگی فرض می‌کنیم که سامانه یک صف دارد و سپس نحوه بسط نتیجه به چندین صف را توضیح می‌دهیم.
...
\section{استراتژی تخلیه‌ی وظیفه بهینه}
\label{sec:strat}
...
\end{equation}
بنابراین جهت یافتن استراتژی بهینه برای یک سیستم تخلیه‌ی وظیفه کافی است که مسئله برنامه‌ریزی خطی حاصل از
$\mathcal{P}_2$
...
\begin{latin}
	\begin{algorithm}
		\rl{\caption{الگوریتم جستجوی استراتژی تخلیه‌ی وظیفه بهینه}\label{alg:cap}}
		\begin{algorithmic}[1]
...
$\eta_0, \cdots, \eta_k$
را به یک هسته یا گره پردازشی خاص اختصاص داد. در شبیه‌سازی سناریو وظایف سبک و سنگین (رجوع شود به \ref{sub:heavylight}) مشاهده شد که الگوریتم موازی‌سازی شده هنگام اجرا بر روی سروری با ۲۴ هسته و تقسیم‌بندی به ۲۴ رشته\LTRfootnote{Thread} عملکردی معادل ۲۰ برابر سریع‌تر از حالت تک‌رشته\LTRfootnote{Single-thread} دارد.

\begin{latin}
	\begin{algorithm}
		\rl{\caption{الگوریتم جستجوی استراتژی تخلیه‌ی وظیفه بهینه موازی‌سازی شده}\label{alg:conc}}
		\begin{algorithmic}[1]
			\Require $precision \geq 2$
